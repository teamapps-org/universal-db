...
public class TransactionStore implements TransactionIdHandler {
	...
	private BlockEntryAtomicStore schemaLogIndex;
	private PrimitiveEntryAtomicStore schemaLogTimestampIndex;
	private PrimitiveEntryAtomicStore schemaLogTransactionIdIndex;
	...
	public void saveSchema(Schema schema, boolean update) throws IOException {
		...
		if (update || schemaLogIndex.getBytes(1) == null) {
			int updateId = 1;
			while (schemaLogIndex.getBytes(updateId) != null) {
				updateId++;
			}
			schemaLogIndex.setBytes(updateId, bytes);
			schemaLogTimestampIndex.setLong(updateId, System.currentTimeMillis());
			schemaLogTransactionIdIndex.setLong(updateId, currentTransactionId);
		}
	}

	public List<Schema> getSchemaLogs() throws IOException {
		List<Schema> schemaUpdates = new ArrayList<>();
		int updateId = 1;
		while (schemaLogIndex.getBytes(updateId) != null) {
			byte[] bytes = schemaLogIndex.getBytes(updateId);
			schemaUpdates.add(new Schema(bytes));
			updateId++;
	...
	public List<Long> getSchemaLogTimestamps() {
		List<Long> timestamps = new ArrayList<>();
		int updateId = 1;
		while (schemaLogTimestampIndex.getLong(updateId) > 0) {
			timestamps.add(schemaLogTimestampIndex.getLong(updateId));
			updateId++;
		}
		return timestamps;
	}

	public List<Long> getSchemaLogTransactionIds() {
		List<Long> transactionIds = new ArrayList<>();
		int updateId = 1;
		while (schemaLogTransactionIdIndex.getLong(updateId) > 0) {
			transactionIds.add(schemaLogTransactionIdIndex.getLong(updateId));
			updateId++;
		}
